...
function Map({ data, homes }) {
  ...
  const [mapCenter, setMapCenter] = useState(
    homes || {
      lat: data[parseInt(data.length / 2)].Longitud,
      lng: data[parseInt(data.length / 2)].Latitud,
    }
  );
  ...
  return isLoaded ? (
    <main style={{ width: "100vw", height: "90vh" }}>
      ...
      {modal && (
        <div className="event-modal" style={{ backgroundColor: "yellow" }}>
          <ul className="event-modal-labels">
            {Object.keys(modalInfo).map((e, index) => (
              <li className="event-modal-list-labels" id={`label${index}`}>
                {t(`forms.${e}`)}
              </li>
            ))}
          </ul>
          <ul className="event-modal-values">
            {Object.values(modalInfo).map((e, index) => (
              <li className="event-modal-list-labels" id={`value${index}`}>
                {e}
              </li>
            ))}
          </ul>
          <button type="button" onClick={() => console.log("apuntarse")}>
            {t("forms.signEvent")}
          </button>
